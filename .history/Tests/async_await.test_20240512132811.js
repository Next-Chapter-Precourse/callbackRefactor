...
describe('getUserDataAsync', () => {
  it('should return user data for existing user', async () => {
    jest.spyOn(global, 'fetchUserDataPromise').mockResolvedValue({ name: "Jane Doe", age: 30 });
  ...
  it('should throw an error for non-existing user', async () => {
    jest.spyOn(global, 'fetchUserDataPromise').mockRejectedValue(new Error('User not found'));
